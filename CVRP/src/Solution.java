...
class Solution
{
    int NoOfVehicles; // количество машин
    int NoOfCustomers; // количество клиентов
    Vehicle[] Vehicles; // Массив транпорта
    double Cost; // Цена проезда
    ...
    {
        this.NoOfVehicles = VechNum; //Количестов машин
        this.NoOfCustomers = CustNum; // Количество клиентов
        this.Cost = 0; // Цена проезда
        Vehicles = new Vehicle[NoOfVehicles]; // Массив машин
        VehiclesForBestSolution =  new Vehicle[NoOfVehicles]; // Массив лучших проездов
        PastSolutions = new ArrayList<>(); // Массив предыдущих решений
    ...
    public void GreedySolution(Node[] Nodes , double[][] CostMatrix) {
        ...
        while (UnassignedCustomerExists(Nodes)) {
            ...
            if ( Candidate  == null)
            {
                ...
                if ( VehIndex+1 < Vehicles.length ) // Необходимо выделить еще ТС
                {
